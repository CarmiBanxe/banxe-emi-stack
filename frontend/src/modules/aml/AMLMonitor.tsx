...
import { ArrowUpRight, X as CloseIcon, Filter, Users, XCircle } from "lucide-react";
import { useState } from "react";
import { type AMLAlert, AMLAlertPanel, type Severity } from "../../components/ui/AMLAlertPanel";
import { type BatchAction, type Column, DataTable } from "../../components/ui/DataTable";
...
function RiskHeatmap({ rows }: { rows: AMLTableRow[] }) {
  const bySeverity = Object.entries(
    rows.reduce<Record<Severity, number>>(
      (acc, r) => {
  ...
  return (
    ...
    >
      <h3 className="text-xs uppercase tracking-widest font-semibold text-[oklch(65%_0_0)] mb-3">Risk Distribution</h3>
      <div className="space-y-2" role="list">
        {bySeverity.map(([severity, count]) => (
          <div key={severity} className="flex items-center gap-3" role="listitem">
            <span className="text-xs font-bold w-16 shrink-0" style={{ color: SEVERITY_DOT[severity as Severity] }}>
              {severity}
            </span>
            ...
            <span className="text-xs tabular-nums text-[oklch(65%_0_0)] w-4 text-right">{count}</span>
          </div>
        ))}
      </div>
    </div>
...
function CaseDetailPanel({ alert, onClose }: { alert: AMLTableRow | null; onClose: () => void }) {
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="rounded-lg border border-[oklch(20%_0.01_240)] bg-[oklch(15%_0.01_240)] p-3 space-y-2">
          <p className="text-xs uppercase tracking-widest font-semibold text-[oklch(45%_0_0)]">Case Info</p>
...
export function AMLMonitor() {
  ...
  const filteredAlerts = MOCK_ALERTS.filter((a) => severityFilter === "ALL" || a.severity === severityFilter)
  ...
  const batchActions: BatchAction[] = [
    {
      label: "Escalate",
      icon: ArrowUpRight,
      // biome-ignore lint/suspicious/noConsole: mock placeholder — wire to HITL escalation service
    ...
    },
    {
      label: "Assign",
      icon: Users,
      // biome-ignore lint/suspicious/noConsole: mock placeholder — wire to officer assignment
      onClick: (ids) => console.log("Assign:", ids),
    },
    {
      label: "Close",
      icon: XCircle,
      // biome-ignore lint/suspicious/noConsole: mock placeholder — wire to case management
  ...
  const columns: Column<AMLTableRow>[] = [
    ...
    },
    {
      ...
      header: "Timestamp",
      width: "150px",
      render: (v) => <span className="font-mono text-xs text-[oklch(45%_0_0)]">{String(v)}</span>,
    },
    {
      key: "status",
      header: "Status",
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "oklch(10% 0 0)" }}>
      ...
      <main className="flex-1 overflow-y-auto flex flex-col">
        ...
        >
          <div>
            ...
            <p className="text-xs text-[oklch(45%_0_0)]">{filteredAlerts.length} alerts — sorted by severity</p>
          </div>
      ...
      {selectedAlert && (
        <>
          <div className="fixed inset-0 bg-black/40 z-30" onClick={() => setSelectedAlert(null)} aria-hidden="true" />
          <CaseDetailPanel alert={selectedAlert} onClose={() => setSelectedAlert(null)} />
        </>
      )}
    </div>
  );
