...
describe("KPICard", () => {
  ...
  test("shows positive delta", () => {
    render(<KPICard label="Test" value="100" delta={5.2} deltaDirection="up" />);
    expect(screen.getByText(/5\.2%/)).toBeInTheDocument();
  });

  test("shows negative delta", () => {
    render(<KPICard label="Test" value="100" delta={-3.1} deltaDirection="down" />);
    expect(screen.getByText(/-3\.1%/)).toBeInTheDocument();
  });

  test("shows delta label", () => {
    render(<KPICard label="Test" value="100" delta={1.0} deltaDirection="up" deltaLabel="vs last week" />);
  ...
  test("up delta has accessible aria-label", () => {
    render(<KPICard label="Test" value="100" delta={5.0} deltaDirection="up" deltaLabel="period" />);
