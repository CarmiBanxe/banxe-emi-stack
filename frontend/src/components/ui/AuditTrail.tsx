 ...
 */
import { Activity, Clock, Lock, User } from "lucide-react";
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xs font-bold uppercase tracking-widest text-[oklch(65%_0_0)]">Audit Trail</h2>
      ...
      >
        {isLoading ? (
          ...
          <p className="px-4 py-8 text-center text-xs text-[oklch(45%_0_0)]">No audit entries found.</p>
        ) : (
          <ol className="divide-y divide-[oklch(17%_0.01_240)]" reversed>
            {displayed.map((entry) => (
              <li key={entry.id} className="px-4 py-2 hover:bg-[oklch(15%_0.01_240)] transition-colors">
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 font-mono text-xs">
                  {/* Timestamp */}
                  <time dateTime={entry.timestamp} className="text-[oklch(45%_0_0)] shrink-0">
                  ...
                  <span className="flex items-center gap-1 text-[oklch(55%_0_0)]">
                    ...
                    <span aria-label={`User: ${entry.userId}`}>{maskUserId(entry.userId)}</span>
                  </span>
                  ...
                  <span className="text-[oklch(65%_0_0)]">
                    {entry.resource}
                    {entry.resourceId && <span className="text-[oklch(45%_0_0)] ml-1">[{entry.resourceId}]</span>}
                  </span>
                  ...
                  {/* Details */}
                  {entry.details && <span className="text-[oklch(45%_0_0)] italic">— {entry.details}</span>}
                </div>
              </li>
            ))}
          </ol>
      ...
      {entries.length > maxEntries && (
        <div
          ...
          "
        >
          Showing {maxEntries} of {entries.length} entries. Export full log for complete audit history.
        </div>
      )}
    </section>
  );
