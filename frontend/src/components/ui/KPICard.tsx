...
function DeltaIndicator({ delta, direction, label }: { delta: number; direction: DeltaDirection; label?: string }) {
  const icons = {
...
export function KPICard({
  ...
  const sparklineColor = deltaDirection === "up" ? "#10b981" : deltaDirection === "down" ? "#f43f5e" : "#3b82f6";
  ...
  return (
    ...
    >
      <header className="flex items-start justify-between">
        <p className="text-xs uppercase tracking-widest font-semibold text-[oklch(65%_0_0)]">{label}</p>
      ...
      <p className="text-3xl font-bold tabular-nums text-[oklch(95%_0_0)] leading-tight">
        {currency && <span className="text-lg font-medium text-[oklch(65%_0_0)] mr-1">{currency}</span>}
        {value}
      </p>

      {delta !== undefined && (
        <footer>
          <DeltaIndicator delta={delta} direction={deltaDirection} label={deltaLabel} />
        </footer>
      )}
    </article>
  );
