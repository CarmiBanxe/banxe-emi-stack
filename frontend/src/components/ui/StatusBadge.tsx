...
import { AlertTriangle, Clock, Search, Shield, X } from "lucide-react";
...
const badgeVariants = cva(
  "inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wide border",
  {
    variants: {
      status: {
        APPROVED: ["bg-[#10b981]/10", "text-[#34d399]", "border-[#10b981]/30"],
        PENDING: ["bg-[#f59e0b]/10", "text-[#fbbf24]", "border-[#f59e0b]/30"],
        REJECTED: ["bg-[#f43f5e]/10", "text-[#f87171]", "border-[#f43f5e]/30"],
        FLAGGED: ["bg-[#f97316]/10", "text-[#fb923c]", "border-[#f97316]/30"],
        UNDER_REVIEW: ["bg-[#3b82f6]/10", "text-[#60a5fa]", "border-[#3b82f6]/30"],
      },
    },
    defaultVariants: {
      status: "PENDING",
...
export function StatusBadge({
  ...
  return (
    <span className={badgeVariants({ status, className })} role="status" aria-label={ariaLabel ?? `Status: ${label}`}>
      {showIcon && <Icon size={12} className="shrink-0" aria-hidden="true" />}
