...
export function AMLAlertPanel({ alert, onReview, compact = false, className }: AMLAlertPanelProps) {
      ...
      <div className={`px-4 ${compact ? "py-3" : "py-4"}`}>
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-2">
            <AlertTriangle size={14} style={{ color: styles.borderColor }} aria-hidden="true" />
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: styles.textColor }}>
              {styles.label}
            </span>
            {alert.ruleId && <span className="text-xs text-[oklch(45%_0_0)] font-mono">[{alert.ruleId}]</span>}
          </div>
        ...
        {/* Title */}
        <p className="text-sm font-semibold text-[oklch(95%_0_0)] mb-1">{alert.title}</p>

        {/* Description */}
        {!compact && <p className="text-xs text-[oklch(65%_0_0)] mb-3 leading-relaxed">{alert.description}</p>}

        {/* Amount + CTA */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {alert.accountId && <span className="text-xs font-mono text-[oklch(45%_0_0)]">{alert.accountId}</span>}
            {alert.amount && alert.currency && (
              <span className="text-xs font-semibold tabular-nums" style={{ color: styles.textColor }}>
