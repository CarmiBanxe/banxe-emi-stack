...
export function Sidebar({
  ...
  return (
    ...
    >
      {/* Logo / Brand */}
      <div
        className="flex items-center px-4 border-b border-[oklch(20%_0.01_240)]"
        style={{ height: '56px' }}
      >
        <div
          className="w-8 h-8 rounded-lg bg-[#3b82f6] flex items-center justify-center shrink-0"
          aria-hidden="true"
        >
          <span className="text-white font-bold text-sm">B</span>
        </div>
        {expanded && (
          <span className="ml-3 font-bold text-[oklch(95%_0_0)] text-sm truncate">
            BANXE
          </span>
        )}
      </div>
          ...
          return (
            <li key={item.id}>
              ...
              >
                ...
                <Icon
                  size={18}
                  className={`shrink-0 ${isActive ? 'text-[#60a5fa]' : ''}`}
                  aria-hidden="true"
                />
