...
export function DataTable<T extends { id: string }>({
  ...
  if (isLoading) {
    return (
      <div className="rounded-xl border border-[oklch(20%_0.01_240)] overflow-hidden" aria-busy="true" aria-label={ariaLabel}>
        {[...Array(5)].map((_, i) => (
          <div key={i} className="flex gap-4 px-4 animate-pulse border-b border-[oklch(17%_0.01_240)]" style={{ height: '44px', alignItems: 'center' }}>
            <div className="w-4 h-4 rounded bg-[oklch(20%_0.01_240)]" />
            {columns.map(col => (
              <div key={String(col.key)} className="h-3 rounded bg-[oklch(20%_0.01_240)]" style={{ width: col.width ?? '120px' }} />
            ))}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="relative">
        ...
        <table className="w-full text-sm" style={{ fontVariantNumeric: 'tabular-nums' }}>
          <thead>
            ...
            >
              ...
              {inlineActions && (
                <th className="px-4 text-xs uppercase tracking-widest font-semibold text-[oklch(65%_0_0)] text-right" scope="col">
                  Actions
                </th>
              )}
            </tr>
      ...
      {batchActions && selected.size > 0 && (
        ...
        >
          <span className="text-sm text-[oklch(65%_0_0)]">
