...
_APPROVAL_LOW_WATERMARK = 0.15    # if approval_rate < this → almost always rejected, auto-reject candidate
...
class FeedbackLoopAnalyser:
    ...
    def _analyse_by_reason(self, corpus: list[dict]) -> list[ReasonStats]:
        ...
        for reason, records in sorted(buckets.items()):
            ...
            fraud_scores = [r.get("fraud_score", 0) for r in records if r.get("fraud_score") is not None]
            amounts = [Decimal(str(r.get("amount", "0"))) for r in records]

            result.append(ReasonStats(
                reason=reason,
                total=len(records),
                approved=approved,
                rejected=rejected,
    ...
    def _analyse_risk_buckets(self, corpus: list[dict]) -> list[RiskBucketStats]:
        ...
        for bmin, bmax, label in bucket_defs:
            records = [
                r for r in corpus
                if bmin <= int(r.get("fraud_score", 0)) <= bmax
            ]
    ...
    def _analyse_amount_buckets(self, corpus: list[dict]) -> list[AmountBucketStats]:
        ...
        for bmin, bmax, label in bands:
            records = [
                r for r in corpus
                if bmin <= Decimal(str(r.get("amount", "0"))) <= bmax
            ]
            if len(records) < _MIN_SAMPLE_STATS:
                continue
            approved = sum(1 for r in records if r.get("outcome") == "APPROVE")
            ...
            terminal = approved + rejected
            approval_rate = (approved / terminal) if terminal > 0 else 0.0
            result.append(AmountBucketStats(
                bucket_label=label,
                total=len(records),
                approved=approved,
                rejected=rejected,
    ...
    def _propose_thresholds(
        ...
        if rs_edd and rs_edd.terminal_count >= self._min_sample and rs_edd.approval_rate > self._high:
            proposals.append(ThresholdProposal(
                proposal_id=_next_id(),
                ...
                evidence=(
                    f"Sample: {rs_edd.total} cases | "
                    f"Avg amount: £{rs_edd.avg_amount:,.2f}"
                ),
        ...
        if rs_fraud and rs_fraud.terminal_count >= self._min_sample and rs_fraud.approval_rate < self._low:
            proposals.append(ThresholdProposal(
                proposal_id=_next_id(),
        ...
        if rs_struct and rs_struct.terminal_count >= self._min_sample and rs_struct.approval_rate < self._low:
            proposals.append(ThresholdProposal(
                proposal_id=_next_id(),
        ...
        logger.debug(
            ...
            len(proposals), len(reason_stats), len(risk_buckets),
        )
        return proposals
