...
class SendGridAdapter:  # pragma: no cover
    ...
    def __init__(self) -> None:
        ...
        self._from_email = os.environ.get("SENDGRID_FROM_EMAIL", "noreply@banxe.co.uk")
        if not self._api_key:
            raise OSError(
                "SENDGRID_API_KEY not set. Register at sendgrid.com → create API key → set in .env"
            )
    ...
    def send(self, request: NotificationRequest) -> NotificationResult:
        if request.channel != NotificationChannel.EMAIL:
            return NotificationResult(
                ...
                error_message=(f"SendGridAdapter only handles EMAIL, got {request.channel}"),
            )
        ...
        try:
            ...
            subject = request.template_vars.get("subject", request.notification_type.value)
            ...
            raise ImportError(
                "sendgrid package not installed. Add sendgrid>=6.0 to requirements.txt"
            )
