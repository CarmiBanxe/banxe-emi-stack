...
class InMemoryVelocityTracker:
    ...
    def record(self, customer_id: str, amount: Decimal) -> None:
        self._records.setdefault(customer_id, []).append(
            _TxRecord(amount=amount, timestamp=datetime.now(timezone.utc))
        )

    def _recent(self, customer_id: str, since: datetime) -> list[_TxRecord]:
        return [
            r for r in self._records.get(customer_id, [])
...
class TxMonitorService:
    ...
    def __init__(self, velocity_tracker: Optional[VelocityTrackerPort] = None) -> None:
        self._tracker: VelocityTrackerPort = (
            velocity_tracker or InMemoryVelocityTracker()
        )
    ...
    def evaluate(self, req: TxMonitorRequest) -> MonitorResult:
        ...
        if thresholds.is_velocity_daily_breach(
            daily_total + req.amount, daily_count + 1
        ):
        ...
        if thresholds.is_velocity_monthly_breach(
            monthly_total + req.amount, monthly_count + 1
        ):
        ...
        window_total, window_count = self._tracker.get_recent_window(
            req.customer_id, hours=24
        )
        if thresholds.is_structuring_signal(
            window_count + 1, window_total + req.amount
        ):
